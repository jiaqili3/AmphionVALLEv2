...
WAVLM_LARGE_FINTUNED_PATH = "/mnt/petrelfs/hehaorui/data/pretrained_models/wavlm/wavlm_large_finetune.pth"
...
if __name__ == "__main__":
    ...
    for wav_info in tqdm(test_data):
        ...
        # resample to 24k
        reference_wav = librosa.resample(reference_wav_16k, orig_sr=16000, target_sr=24000)
        reference_wav = torch.tensor(reference_wav, dtype=torch.float32).to(device)
        reference_wav_16k = torch.tensor(reference_wav_16k, dtype=torch.float32).to(device)
        ...
        # resample to 16k
        output_wav = torchaudio.functional.resample(output_wav, orig_freq=24000, new_freq=16000)
        
        input_values = asr_processor(output_wav.squeeze(0).squeeze(0), return_tensors="pt").input_values
        ...
        sim_o = extract_wavlm_similarity(output_wav, reference_wav.unsqueeze(0), speaker_encoder)
        print(f"SIM-O: {sim_o}")
        # # FID:
